...
def craft_names(rand_words, snippet, pattern, caps=false)
    ...
    # double due to both key and value has the pattern    
    return names * 2
end


def craft_params(rand_words, snippet, pattern)
    names = (0...snippet.scan(pattern).size).map do
        pattern_count = rand(3) + 1
        params = (0...pattern_count).map {|x| rand_words.pop}
        params.join(', ')
    end
...
loop do 
    ...
    for snippet in snippets 
        ...
        if PHRASE_FIRST
            question, answer = ansswer, question
        end

        print question, "\n\n> "
       
        # hit CTRL-D, gets return nil 
